        ...
        try:
            ...
            for doc in docs:
                try:
                    ...
                    if not chunks:
                        self.logger.info(f"No chunks found for document {doc.readable_filename}")
                        continue

                    self.logger.info(f"Processing {len(chunks)} chunks for document {doc.readable_filename}")
                    ...
                    self.logger.info(f"Processing {len(table_chunks)} table chunks and {len(content_chunks)} content chunks")
                    ...
                    chunk_batches = (
                        ...
                        [[json.dumps(chunk.table_data) if chunk.table_data else chunk.table_html] for chunk in table_chunks] +
                        # Process content chunks in batches
...
"""
                        ...
                        try:
                            # Pass the existing metadata as context for the next extraction
                            existing_metadata = None
                            if all_metadata and len(all_metadata) > 0:
                                existing_metadata = all_metadata[-1]
                            ...
                            result = await self.extractor.ainvoke(
                                {
                                    ...
                                    "existing": {"DocumentMetadata": existing_metadata} if existing_metadata else None
                                }
                            )
